...
We recommend using the [ISO 8601](https://www.iso.org/iso-8601-date-and-time-format.html) format `yyyy-MM-dd` to help:
...
When viewing dates within a plain text file the metadata string `07/03/2005` could be interpreted by the reader as "3rd July 2005" or "7th March 2005" depending on their location or cultural origin.

For the same reason, it is suggested *not* to use the `short` style when formatting date outputs.
...
Using a `short` date style is not recommended.
